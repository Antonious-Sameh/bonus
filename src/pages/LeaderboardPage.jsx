...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        setLoading(true);
        const response = await axios.get('https://bonus-system-tau.vercel.app/api/admin/leaderboard');
  ...
  const sortedCustomers = [...customersList].sort((a, b) => b.points - a.points);

  const getBadgeIcon = (rank) => {
    switch (rank) {
      case 1: return <Trophy className="w-6 h-6 text-yellow-400 drop-shadow-[0_0_8px_rgba(255,215,0,0.5)]" />;
      case 2: return <Medal className="w-6 h-6 text-gray-300 drop-shadow-[0_0_8px_rgba(192,192,192,0.5)]" />;
      case 3: return <Award className="w-6 h-6 text-amber-600 drop-shadow-[0_0_8px_rgba(205,127,50,0.5)]" />;
  ...
  const getRowStyle = (rank) => {
    switch (rank) {
      case 1: return 'border-yellow-400/30 bg-yellow-400/5 hover:border-yellow-400/50';
      case 2: return 'border-gray-300/30 bg-gray-300/5 hover:border-gray-300/50';
      case 3: return 'border-amber-600/30 bg-amber-600/5 hover:border-amber-600/50';
      default: return 'hover:border-primary/30 hover:bg-white/10';
    }
  };
        ...
        <div className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
          ...
          >
            <div>
              ...
              <p className="text-lg text-muted-foreground">أفضل الزباين وأكثرهم نقاطاً</p>
            </div>
            <Button
          ...
          <div className="space-y-4">
            {sortedCustomers.length > 0 ? (
              sortedCustomers.map((customer, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-right">
                    <p className="text-xl font-bold text-foreground truncate">{customer.name}</p>
              ...
              >
                ...
                <p className="text-lg text-foreground font-medium">لسه مفيش زباين</p>
                <p className="text-sm text-muted-foreground mt-2">ابدأ ضيف زباين عشان يظهروا هنا</p>
              </motion.div>
            )}
          </div>
        </div>
